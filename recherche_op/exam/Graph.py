...
class Graph():
    ...
    def get_distances_multiple(self, chemins):
        distances = []
        for chemin in chemins:
            distances.append(self.get_distance(chemin))
        return distances
    ...
    def recuit_simule_with_start(self, start, temperature_initiale=10000, L=5, alpha=0.9, K=1000):
        # set temp
        temperature = temperature_initiale
        # init chemin with greedy one
        chemin_actuel = self.best_chemin_greedy(start)
        # init best chemin by first one
        meilleur_chemin = chemin_actuel
        best_distance = self.get_distance(chemin_actuel)
    ...
    def show_chemin(self, chemin, step=0, color='red'):
        ...
        cities_in_chemin = [self.cities[int(i)] for i in chemin]
    ...
    def cycle_path_to_city(self, path, start=0):
        # find index of start
        start_index = list(path).index(start)
        return list(path[start_index:]) + list(path[:start_index])
    ...
    def get_n_optimal_paths(self, n):
        ...
        for part in parts:
            ...
            path = sub_g.recuit_simule_with_start(
                start=0, temperature_initiale=2000000, L=5, alpha=0.5, K=25000)
            path = self.cycle_path_to_city(path, start=0)
            optimal_paths.append(self.convert_path(part, path))

        return optimal_paths

    def best_n_optimal_paths(self, max_n):
        best_paths = []
        best_distance = np.inf
        for n in range(1, max_n):
            paths = self.get_n_optimal_paths(n)
            distances = self.get_distances_multiple(paths)
            print(f"n={n}, best distance={sum(distances)}")
    ...
    def convert_path(self, local_city_list, path):
        ...
        for city_index in path:
            ...
            i = pos.index(local_city_list[int(city_index)])
            global_path.append(i)
        return global_path

    def show_multiples_paths(self, chemins, step=0, colors=[]):
        if (len(colors) == 0):
