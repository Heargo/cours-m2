...
# g_532.show_chemin(recuit_simule, step=0.01)


paths = g_532.best_n_optimal_paths(5)
# paths = g_532.get_n_optimal_paths(3)
total = sum(g_532.get_distances_multiple(paths))
print(
    f"the most optimal path is with {len(paths)} trucks and is of {total} length")

# save opti chemin to best_path.txt
with open("best_path.txt", "w") as f:
    f.write(str(paths))
c = ['red', 'pink', 'blue', 'yellow', 'orange', 'purple', 'brown', 'black']

g_532.show_multiples_paths(paths, step=0.001, colors=c)
